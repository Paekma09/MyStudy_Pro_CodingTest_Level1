...
public class Main {
	...
	*/
	...
	public static int solution(int[] nums) {
        int answer = 0;
        // N은 nums의 길이이며, 가져갈 수 있는 포켓몬 n은 N / 2이다.
        int N = nums.length;
        int n = N / 2;
        // int배열을 Set으로 변환하기전에 먼저 Integer배열로 변환 해준다.
        Integer[] num = Arrays.stream(nums).boxed().toArray(Integer[]::new); 
        // 중복값을 없애기 위해 변환된 Integer배열을 Set으로 변환 한다.
        Set<Integer> set = new HashSet<>(Arrays.asList(num));
        // Set의 사이즈가 n보다 크거나 같으면 포켓몬은 최대 n종류 만큼 가져 갈 수 있으며 그 반대의 경우에는 Set의 사이즈 만큼 가져 갈 수 있다. 
        if(set.size() >= n) {
        	answer = n;
        }else if(set.size() < n) {
        	answer = set.size();
