...
public class Main {
	...
	입출력 예 #2
		...
		두 번째 사진 속엔 그리워하는 인물이 없으므로 0점입니다. 세 번째 사진 속 그리워하는 사람은 "don"만 있으므로 55점입니다. 
		따라서 [67, 0, 55]를 반환합니다.
	입출력 예 #3
		설명 생략
	 */
	public static void main(String[] args) {
		...
		System.out.println(Arrays.toString(name1)+" "+Arrays.toString(yearning1)+" "+Arrays.deepToString(photo1));
		System.out.println(Arrays.toString(solution(name1, yearning1, photo1)));
		System.out.println(Arrays.toString(name2)+" "+Arrays.toString(yearning2)+" "+Arrays.deepToString(photo2));
		System.out.println(Arrays.toString(solution(name2, yearning2, photo2)));
		System.out.println(Arrays.toString(name3)+" "+Arrays.toString(yearning3)+" "+Arrays.deepToString(photo3));
		System.out.println(Arrays.toString(solution(name3, yearning3, photo3)));
	}
	
	public static int[] solution(String[] name, int[] yearning, String[][] photo) {
		// answer의 길이는 photo의 길이와 같다
        int[] answer = new int[photo.length];
        // name과 yearning은 map의 key와 value로 저장하낟.
        Map<String, Integer> map = new LinkedHashMap<>();
        for(int i=0; i<name.length; i++) {
        	map.put(name[i], yearning[i]);
        }
        // photo배열을 반복문으로 돌며 map의 key를 가지고 있는지 여부 확인 하여 true이면 answer에 가지고 있는 key의 value값을 더해준다.
        for(int i=0; i<photo.length; i++) {
        	for(int j=0; j<photo[i].length; j++) {
        		if(map.containsKey(photo[i][j]) == true) {
